...
class RelationNetworks(FewShotClassifier):
    ...
    def process_support_set(
        ...
        support_features = self.compute_features(support_images).unsqueeze(-1).unsqueeze(-1)
    ...
    def forward(self, query_images: Tensor) -> Tensor:
        ...
        prototype.
        """
        query_features = self.compute_features(query_images).unsqueeze(-1).unsqueeze(-1)
        ...
        relation_scores = self.relation_module(query_prototype_feature_pairs.squeeze()).view(
