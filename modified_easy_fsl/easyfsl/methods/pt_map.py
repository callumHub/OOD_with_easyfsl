...
MAXIMUM_SINKHORN_ITERATIONS = 10000
...
class PTMAP(FewShotClassifier):
    ...
    def compute_features(self, images: Tensor) -> Tensor:
        ...
        features = torch.clamp(features, min=0)
    ...
    def compute_optimal_transport(
        ...
        transport_plan = torch.exp(-self.lambda_regularization * cost_matrix)#.clamp(0,10)
        transport_plan = transport_plan / transport_plan.sum(dim=(0, 1), keepdim=True)
        for _ in range(MAXIMUM_SINKHORN_ITERATIONS):
            transport_plan = transport_plan.clamp(1e-12,10)

            per_class_sums = transport_plan.sum(1)
            transport_plan = transport_plan * (1 / (per_class_sums)).unsqueeze(1)
            transport_plan = transport_plan * (
                instance_multiplication_factor / (transport_plan.sum(0))
            ).unsqueeze(0)

            if torch.max(torch.abs(per_class_sums - transport_plan.sum(1))) < epsilon:
                break
        transport_plan = transport_plan.to("cuda:0")
    ...
    def update_prototypes(self, all_features, all_assignments) -> None:
        ...
        self.prototypes = self.prototypes + (self.fine_tuning_lr * delta)
